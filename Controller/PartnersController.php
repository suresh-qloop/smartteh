...
class PartnersController extends AppController
{
	...
	public function admin_index(): void {
		$data = $this->Partner->adminList($this, 15);
	...
	public function admin_update(int $id): void {
		if ($this->request->is(['post', 'put'])) {
			...
			$id = (int)$this->request->data('Partner.id');
		} else {
			$this->request->data = $this->Partner->getOrFail($id);
		}
	...
	public function admin_active(int $id, bool $enabled = null): void {
		$success = $this->Partner->active($id, $enabled);
	...
	public function admin_moveup(int $id): void {
		$success = $this->Partner->moveup($id);
		$this->actionResponse($success);
	}
	...
	public function admin_movedown(int $id): void {
		$success = $this->Partner->movedown($id);
		$this->actionResponse($success);
	}
